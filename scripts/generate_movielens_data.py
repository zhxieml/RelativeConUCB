...
def load_sparse_matrix(input_folder):
    ...
    artist_to_tagcount = {}
    tag_to_artists = {}
    df = pd.read_csv(os.path.join(input_folder, "user_taggedmovies.dat"), sep="\t", header=0, index_col=None)
    for _, row in tqdm(df.iterrows()):
        if row["movieID"] not in artistID:
            artistID[row["movieID"]] = len(artistID)
        if row["userID"] not in userID:
            userID[row["userID"]] = len(userID)
        if row["tagID"] not in tagID:
            tagID[row["tagID"]] = len(tagID)

        artist = artistID[row["movieID"]]
