...
SIGMA_SUPARM = 1
SIGMA_BASE_USER = 1
MIN_ABS_ARM_REWARD = 0.8
MAX_ABS_ARM_REWARD = 0.8
SIGMA_USER = [0.0, 0.5, 1.0, 2.0, 5.0]
...
if __name__ == "__main__":
    ...
    output = os.path.join(args.output, "synthetic_group")
    if not os.path.exists(output):
        os.mkdir(output)

    # Generate base user feature vectors.
    user_base_feats = np.random.normal(0, SIGMA_BASE_USER, (DIM_FEAT - 1,))

    # Generate different user groups.
    for sigma_user in SIGMA_USER:
        suffix = str(sigma_user)
        user_feats = {}
        ...
        # Save.
        group_output = os.path.join(output, "synthetic_sigma_{}".format(sigma_user))
        if not os.path.exists(group_output):
            os.mkdir(group_output)
        np.save(os.path.join(group_output, "arm_to_suparms"), dict(arm_to_suparms))
        np.save(os.path.join(group_output, "arm_feats"), arm_feats)
        np.save(os.path.join(group_output, "user_base_feats"), user_base_feats)
        np.save(os.path.join(group_output, "affinity"), affinity_matrix)
